...
#define FLASH_DATA_HEADER_SIZE (sizeof(uint32_t) + sizeof(uint32_t))
...
static size_t flash_record_size(uintptr_t base) {
    return 2 * sizeof(uint32_t) + sizeof(uint16_t) + flash_record_length_size(base) + flash_record_payload_size(base);
}
...
static uintptr_t allocate_free_addr(uint32_t size, bool persistent, uintptr_t *publish_at) {
    ...
    size_t overhead = 2 * sizeof(uint32_t) + sizeof(uint16_t) + length_size;
    size_t real_size = 0;
    ...
    *publish_at = 0x0;
    ...
    for (uintptr_t base = endp; base >= startp; base = next_base) {
        ...
        if (potential_addr >= gap_start && potential_addr >= startp) {
            ...
            if (r == PICOKEYS_OK) {
                r = flash_program_uintptr(potential_addr + sizeof(uint32_t), base);
            }
            if (r == PICOKEYS_OK && next_base != 0x0) {
                r = flash_program_uintptr(next_base + sizeof(uint32_t), potential_addr);
            }
            if (r != PICOKEYS_OK) {
                return 0x0;
            }
...
static int flash_write_data_to_file_internal(file_t *file, const_byte_array_t data, uint32_t offset, bool partial) {
    ...
    file_t new_file = {
        .data = (uint8_t *)new_addr + 2 * sizeof(uint32_t) + sizeof(uint16_t),
        .fid = file->fid
    };
    ...
    int r = flash_program_halfword(new_addr + 2 * sizeof(uint32_t), new_file.fid);
    ...
    if (replacing && 2 * sizeof(uint32_t) + sizeof(uint16_t) + length_size + new_size > FLASH_SECTOR_SIZE && !flash_commit_sync(5000u)) {
        return PICOKEYS_ERR_MEMORY_FATAL;
    }
    *file = new_file;
    return PICOKEYS_OK;
