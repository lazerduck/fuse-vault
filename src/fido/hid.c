...
void fv_fido_hid_receive(fv_fido_hid_t *p, const uint8_t r[64], uint32_t now) {
    ...
    if (r[4] == INIT) {
        ...
        if (cid == UINT32_MAX) {
            ...
            if (slot == 4) slot=(p->next_channel-1)%4; /* Reclaim an idle channel. */
...
void fv_fido_hid_tick(fv_fido_hid_t *p, uint32_t now) {
    if (p->receiving && !p->transmitting && (uint32_t)(now - p->rx_time) >= 3000) {
        p->receiving = false; memset(p->rx, 0, sizeof(p->rx));
        error(p, p->active_channel, 5);
    }
}
bool fv_fido_hid_peek(const fv_fido_hid_t *p, uint8_t r[64]) {
...
void fv_fido_hid_sent(fv_fido_hid_t *p) {
    if (!p->transmitting) return;
...
/* Caller hands off immutable rx to its worker, then completes on this owner. */
void fv_fido_hid_complete(fv_fido_hid_t *p,const uint8_t *response,size_t n){
    if(!p->processing)return;
    p->processing=false;p->pending=false;
    if(p->resync_requested){
        ...
        if(p->cancelled){p->tx[0]=0x2d;n=1;}
        else if(!response || !n || n>sizeof(p->tx)){p->tx[0]=0x7f;n=1;}
        else memcpy(p->tx,response,n);
        reply(p,p->active_channel,CBOR,n);
    }
    memset(p->rx,0,sizeof(p->rx));
}
