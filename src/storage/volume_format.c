...
static bool valid(const fv_volume_descriptor *d,uint64_t capacity) {
    if(!d || d->layout_version>2 || !d->logical_blocks || d->layer_count<1 || d->layer_count>FV_MAX_LAYERS)return false;
    ...
    uint64_t bitmap=d->layout_version==2?m/4096+(m%4096!=0):0;
    return capacity>=FV_VOLUME_METADATA_BASE && bitmap<=capacity-FV_VOLUME_METADATA_BASE &&
        m<=capacity-FV_VOLUME_METADATA_BASE-bitmap && n<=capacity-FV_VOLUME_METADATA_BASE-bitmap-m;
}
bool fv_volume_descriptor_encode(const fv_volume_descriptor *d,uint64_t capacity,uint8_t out[128]) {
    ...
    memcpy(out,"FV2VOL01",8);put(out+8,d->layout_version?d->layout_version:1,2);put(out+10,512,2);
    ...
    uint64_t bitmap=d->layout_version==2?m/4096+(m%4096!=0):0;
    put(out+40,d->logical_blocks,8);put(out+48,FV_VOLUME_METADATA_BASE+bitmap,8);put(out+56,m,8);
    put(out+64,FV_VOLUME_METADATA_BASE+bitmap+m,8);
    if(bitmap){put(out+100,FV_VOLUME_METADATA_BASE,8);put(out+108,bitmap,8);}
    put(out+72,16,8);put(out+80,2048,8);
...
bool fv_volume_descriptor_decode(const uint8_t *in,size_t bytes,uint64_t capacity,fv_volume_descriptor *out) {
    ...
    d.layout_version=(uint16_t)get(in+8,2);if(d.layout_version!=1 && d.layout_version!=2)return false;
...
uint64_t fv_volume_max_blocks(uint64_t capacity) {
    if(capacity<=FV_VOLUME_METADATA_BASE)return 0;
    uint64_t available=capacity-FV_VOLUME_METADATA_BASE;
    uint64_t low=0,high=available<UINT32_MAX?available:UINT32_MAX;
    while(low<high){
        uint64_t n=low+(high-low+1)/2,m=n/15+(n%15!=0),b=m/4096+(m%4096!=0);
        if(n+m+b<=available)low=n;else high=n-1;
    }
    return low;
}
