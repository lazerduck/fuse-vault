...
#ifndef FV_SD_CLOCK_HZ
#define FV_SD_CLOCK_HZ 25000000u
#endif
static sd_sdio_if_t native_if = {
    .CMD_gpio = FUSE_VAULT_SD_CMD_PIN, .D0_gpio = FUSE_VAULT_SD_DAT0_PIN,
    .SDIO_PIO = pio1, .DMA_IRQ_num = DMA_IRQ_1, .baud_rate = FV_SD_CLOCK_HZ,
};
...
static fv_block_result_t validate(fv_block_device_t *device, uint64_t first,
    ...
    if (!device || !device->context || !buffer || !count || ((uintptr_t)buffer & 3u)) return FV_BLOCK_ERROR_INVALID_ARGUMENT;
