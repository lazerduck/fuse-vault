...
static uint8_t fv_rp_walk_hash[32];
void fv_pico_cred_session_clear(void) {
    rp_walk_active = cred_walk_active = false;
    rp_channel = cred_channel = rp_timer = cred_timer = 0;
    rp_counter = cred_counter = 1; rp_total = cred_total = 0;
    memset(fv_rp_walk_hash, 0, sizeof(fv_rp_walk_hash));
    memset(&rpIdHashx, 0, sizeof(rpIdHashx));
}
...
int cbor_cred_mgmt(const uint8_t *data, size_t len) {
    ...
    bool is_preview = *(data - 1) == 0x41, pinUvAuthProtocol_present = false; // Backwards compatibility
    ...
    if (subcommand == 0x01) {
        ...
        if (error != CborNoError) {
            fv_pico_log("Cannot encode CBOR [%s:%d]: cbor_encode_text_string(&mapEncoder2, (char *) rp_record.id, rp_record.id_len) (%d)\n", __FILE__, __LINE__, error);
            goto err;
        }
        ...
        if (cred_counter <= cred_total) {
            memcpy(fv_rp_walk_hash, rpIdHash.data, sizeof(fv_rp_walk_hash));
            rpIdHashx = (CborByteString){.data=fv_rp_walk_hash, .len=32, .present=true, .nofree=true};
