...
int cbor_client_pin(const uint8_t *data, size_t len) {
    ...
    if (fv_pico_has_uv() && subcommand != 2 && subcommand != 6 && subcommand != 7) {
    ...
    if (subcommand == 0x0) {
    ...
    else if (subcommand == 0x7 && fv_pico_has_uv()) {
        CBOR_CHECK(cbor_encoder_create_map(&encoder, &mapEncoder, 1));
        CBOR_CHECK(cbor_encode_uint(&mapEncoder, 5));
        CBOR_CHECK(cbor_encode_uint(&mapEncoder, fv_pico_uv_retries()));
    }
    else if (subcommand == 0x6 && fv_pico_has_uv()) {
        if (!kax.present || !kay.present || kax.len != 32 || kay.len != 32 ||
            kty != 2 || alg != -25 || crv != 1 || permissions == 0 ||
            ((permissions & (CTAP_PERMISSION_MC | CTAP_PERMISSION_GA)) && !rpId.present)) {
            CBOR_ERROR(CTAP1_ERR_INVALID_PARAMETER);
        }
        if (permissions & ~(uint64_t)(CTAP_PERMISSION_MC | CTAP_PERMISSION_GA | CTAP_PERMISSION_CM)) {
            CBOR_ERROR(CTAP2_ERR_UNAUTHORIZED_PERMISSION);
        }
        uint8_t rp_hash[32] = {0}, shared[64] = {0}, token[32 + IV_SIZE] = {0};
        if (rpId.present && mbedtls_sha256((uint8_t *)rpId.data, rpId.len, rp_hash, 0) != 0) {
            CBOR_ERROR(CTAP2_ERR_PROCESSING);
        }
        if (!fv_pico_verify_user(rpId.present ? rp_hash : NULL)) {
            CBOR_ERROR(CTAP2_ERR_UV_BLOCKED);
        }
        if (mbedtls_mpi_read_binary(&hkey.ctx.mbed_ecdh.Qp.X, kax.data, kax.len) != 0 ||
            mbedtls_mpi_read_binary(&hkey.ctx.mbed_ecdh.Qp.Y, kay.data, kay.len) != 0 ||
            ecdh((uint8_t)pinUvAuthProtocol, &hkey.ctx.mbed_ecdh.Qp, shared) != 0) {
        ...
        paut.has_rp_id = rpId.present;
        if (rpId.present) memcpy(paut.rp_id_hash, rp_hash, sizeof(rp_hash));
        int encrypted = encrypt((uint8_t)pinUvAuthProtocol, shared, paut.data, 32, token);
        mbedtls_platform_zeroize(shared, sizeof(shared));
        if (encrypted != 0) {
        ...
        if (error == CborNoError) error = cbor_encode_uint(&mapEncoder, 2);
        if (error == CborNoError) error = cbor_encode_byte_string(&mapEncoder, token, 32u + (pinUvAuthProtocol == 2 ? IV_SIZE : 0u));
        mbedtls_platform_zeroize(token, sizeof(token));
        if (error != CborNoError) goto err;
    }
...
/* Fuse Vault port: destroy session ECDH and verification state on mode exit. */
void fv_pico_pin_session_clear(void) {
    if (hkey_init) mbedtls_ecdh_free(&hkey);
    hkey_init = false;
    needs_power_cycle = false;
    usage_timer = initial_usage_time_limit = 0;
}
