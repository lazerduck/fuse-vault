...
static fv_journal_result_t decode_record(
    ...
    *state = (fv_journal_state_t) {
    ...
    if (read_u32(record + 8u) >= 2u) {
        if (record[50] > 1u || (read_u32(record + 8u) == 2u && record[50] != 1u)) return FV_JOURNAL_AUTHENTICATION_ERROR;
        state->fido_initialized = record[50] == 1u;
        memcpy(state->fido_digest, record + 51u, 32u);
    }
    if (read_u32(record + 8u) == 3u) {
        state->header_sequence = read_u64(record + 83u);
        if (state->header_sequence == 0u) return FV_JOURNAL_AUTHENTICATION_ERROR;
        memcpy(state->header_tag, record + 91u, 32u);
    }
...
static bool encode_record(fv_security_journal_t *journal,
    ...
    write_u32(record + 8u, state->header_sequence ? 3u : state->fido_initialized ? 2u : JOURNAL_FORMAT_VERSION);
    ...
    if (state->header_sequence) {
        write_u64(record + 83u, state->header_sequence);
        memcpy(record + 91u, state->header_tag, 32u);
    }
...
fv_journal_result_t fv_security_journal_append(
    ...
    } else if ((latest.header_sequence &&
                (state->header_sequence < latest.header_sequence ||
                 (state->header_sequence != latest.header_sequence &&
                  (latest.header_sequence == UINT64_MAX ||
                   state->header_sequence != latest.header_sequence + 1u)) ||
                 (state->header_sequence == latest.header_sequence &&
                  !constant_time_equal(state->header_tag, latest.header_tag, 32u)))) ||
               (latest.fido_initialized && !state->fido_initialized) ||
