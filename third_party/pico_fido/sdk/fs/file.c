...
static void scan_region(bool persistent) {
    ...
    for (uintptr_t base = flash_read_uintptr(endp); base >= startp; base = flash_read_uintptr(base)) {
        ...
        uint16_t fid = flash_read_uint16(base + sizeof(uint32_t) + sizeof(uint32_t));
        uintptr_t length_addr = base + 2 * sizeof(uint32_t) + sizeof(uint16_t);
        ...
        size_t header_size = stored_length == FLASH_FILE_EXTENDED_LENGTH ? 16u : 12u;
        if (endp - base < header_size || length > endp - base - header_size) {
            fv_pico_engine_fail(); return;
        }
        fv_pico_log("[%x] scan fid %x, len %lu\n", (unsigned int)base, fid, (unsigned long)length);
        file_t *file = (file_t *) file_search_by_fid(fid, NULL, SPECIFY_EF);
        if (!file) {
            file = file_new(fid);
        }
        if (file) {
            file->data = (uint8_t *) (base + sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint16_t));
        }
        if (!persistent) {
            num_files++;
        }
...
void file_scan_flash(void) {
    ...
    uint32_t r2 = (uint32_t)flash_read_uintptr(end_rom_pool + sizeof(uint32_t));
    if ((r1 == 0xffffffff || r1 == 0xefefefef) && (r2 == 0xffffffff || r2 == 0xefefefef)) {
        fv_pico_log("First initialization (or corrupted!)\n");
        uint8_t empty[sizeof(uint32_t) * 2 + sizeof(uint32_t)];
...
int flash_clear_file(file_t *file) {
    ...
    uintptr_t base_addr = (uintptr_t)(file->data - sizeof(uint32_t) - sizeof(uint16_t) - sizeof(uint32_t));
    uintptr_t prev_addr = flash_read_uintptr(base_addr + sizeof(uint32_t));
    uintptr_t next_addr = flash_read_uintptr(base_addr);
    //fv_pico_log("nc %lx->%lx   %lx->%lx\n",prev_addr,flash_read_uintptr(prev_addr),base_addr,next_addr);
    ...
    if (next_addr > 0) {
        flash_program_uintptr(next_addr + sizeof(uint32_t), prev_addr);
    }
    flash_program_uintptr(base_addr, 0);
    flash_program_uintptr(base_addr + sizeof(uint32_t), 0);
    file->data = NULL;
    if (num_files > 0) {
        num_files--;
    }
