...
int cbor_make_credential(const uint8_t *data, size_t len) {
    ...
    for (unsigned int i = 0; i < pubKeyCredParams_len; i++) {
        ...
        if (pubKeyCredParams[i].alg != FIDO2_ALG_ES256) continue;
    ...
    if (fv_pico_has_uv()) {
        if (!fv_pico_verify_user(rp_id_hash)) { CBOR_ERROR(CTAP2_ERR_UV_BLOCKED); }
        flags |= FIDO2_AUT_FLAG_UV;
    }
    if (options.present) {
        if (options.uv == ptrue && !fv_pico_has_uv()) { //5.3
            CBOR_ERROR(CTAP2_ERR_INVALID_OPTION);
        }
        if (options.rk != NULL) {
    ...
    if (extensions.present == true) {
        ...
        if (l > 0) {
            ...
            if (uvm == ptrue) {
                ...
                CBOR_CHECK(cbor_encode_uint(&uvm_entry, fv_pico_has_uv() ? 0x00000004 : 0x00000800)); // passcode_external
    ...
    *pa++ = flags;
    ...
    /* V2 uses constant-zero signature counters. */
    flash_commit();
err:
